...
// Function to shuffle the Rubik's Cube
function shuffleRubiksCube(cube, numMoves) {
...
// Function to apply a single move to the Rubik's Cube
function applyMove(cube, move) {
  const temp = JSON.parse(JSON.stringify(cube)); // Create a temporary copy of the cube
  const face = cube[1]; // Middle layer, which remains stationary

  // Define the move logic for each move (U, D, L, R, F, B)
  // This is a simplified example and doesn't cover all possible moves
  switch (move) {
    case 'U':
      cube[0] = rotateClockwise(temp[0]);
      break;
    case 'D':
      cube[2] = rotateClockwise(temp[2]);
      break;
    case 'L':
      for (let i = 0; i < 3; i++) {
        cube[i][0] = temp[i][2];
...
document.getElementById("shuffle-button").addEventListener("click", function() {
  shuffleRubiksCube(rubiksCube, 20); // Shuffle the cube
  displayRubiksCube(); // Display the shuffled cube on the page
});

// Function to display the Rubik's Cube on the page
function displayRubiksCube() {
  const cubeDiv = document.getElementById("rubiks-cube");
  cubeDiv.innerHTML = ""; // Clear the previous content

  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      const face = rubiksCube[i][j];
      const faceDiv = document.createElement("div");
      faceDiv.className = "face";
      faceDiv.textContent = face.join("\n"); // Display the face content
